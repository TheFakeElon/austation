...
/obj/structure/mechanical/flywheel/proc/get_energy(isolated = FALSE)
	var/energy = 0.5 * angular_mass * RPM_TO_RADS(rpm) ** 2
	if(!additional.len || isolated)
		return energy

	var/t_energy = energy
	for(var/obj/structure/mechanical/flywheel/F in additional)
		t_energy += F.get_energy(TRUE)

	return t_energy

/// Probably will be used for something
/obj/structure/mechanical/flywheel/proc/get_centrif_force()
...
/obj/structure/mechanical/flywheel/proc/overstress(warning = TRUE)
	if(!warning)
		footloose()
	else
		addtimer(CALLBACK(src, .proc/footloose), rand(10, 50))

/obj/structure/mechanical/flywheel/proc/footloose()
	set waitfor = FALSE
...
/obj/structure/mechanical/flywheel/proc/suck_energy(joules, isolated = FALSE)
	if(!bearing)
		return FALSE
